...
struct tPrData {
  ...
  vector<int> fps;         // flag indicate if it is fp
  vector<int> fns;         // flag indicate if it is fn
  tPrData () :
...
vector<string> generateFileNames(int32_t img_idx,double threshold,string root_dir){
    vector<string> filenames;

    char file_name[256];
    sprintf(file_name,root_dir.c_str());
    sprintf(file_name,"/fps");
    sprintf(file_name,"/%f",threshold);
    sprintf(file_name,"/%06d.txt",img_idx);

    char file_name2[256];

    sprintf(file_name2,root_dir.c_str());
    sprintf(file_name2,"/fns");
    sprintf(file_name2,"/%f",threshold);
    sprintf(file_name2,"/%06d.txt",img_idx);

    filenames.push_back(string(file_name));
    filenames.push_back(string(file_name2));
    return filenames;
}

void saveIdxInfo(tPrData tmp,int32_t img_idx, double threshold, string root_dir){
    vector<string> filenames = generateFileNames(img_idx,threshold,root_dir);
    ...
    for(int i=0;i<tmp.fps.size();i++){
        ...
        // put all different difficulty into one file
        fprintf(fp,"\n\n");
    }
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
        ...
        DIFFICULTY difficulty, METRIC metric,string analysis_dir = "") {
  ...
  for (int32_t i=0; i<groundtruth.size(); i++){
    ...
    for(int32_t t=0; t<thresholds.size(); t++){
      ...
      saveIdxInfo(tmp,i,thresholds[i],analysis_dir);
...
bool eval(string result_sha,Mail* mail){
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      ...
      if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[0], aos[0], EASY, IMAGE,analysis_dir)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[1], aos[1], MODERATE, IMAGE,analysis_dir)
         || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, imageBoxOverlap, precision[2], aos[2], HARD, IMAGE,analysis_dir)) {
