...
// mock mini overlaps for analysis
const double MIN_OVERPLAP_LEVELS[] = {0.6,0.5,0.4};
...
struct tPrData {
    ...
    vector<int> fps;         // idx if it is fp
    vector<int> fns;         // idx if it is fn
    vector<int> fns_bst;    // idx  if its overlap is near to min_overlap in fns
    vector<int> fps_bst;    // idx if its overlap is near to min_overlap in fps
    tPrData () :
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
    ...
    for(int32_t i=0; i<gt.size(); i++){
        ...
        /*=======================================================================
          ...
          =======================================================================*/
        int32_t det_idx          = -1;
        int32_t mock_det_idx = -1;
        double valid_detection = NO_DETECTION;
        double max_overlap     = 0;
        double mock_max_overlap =0;
        ...
        for(int32_t j=0; j<det.size(); j++){
            ...
            else if(compute_fp && (overlap>max_overlap || assigned_ignored_det) && ignored_det[j]==0){
                if(overlap>MIN_OVERLAP[metric][current_class]){
                 max_overlap     = overlap;
                det_idx         = j;
                valid_detection = 1;
                assigned_ignored_det = false;
                }else if(overlap>MIN_OVERPLAP_LEVELS[0]){
                    mock_max_overlap = overlap;
                    mock_det_idx = j;
                }
                           }
            else if(compute_fp &&valid_detection==NO_DETECTION && ignored_det[j]==1){
                if(overlap>MIN_OVERLAP[metric][current_class]){
                det_idx              = j;
                valid_detection      = 1;
        ...
        if(valid_detection==NO_DETECTION && ignored_gt[i]==0) {
            stat.fns.push_back(i);
            if(mock_max_overlap>MIN_OVERPLAP_LEVELS[0]){
                stat.fns_bst.push_back(i);
            }
            stat.fn++;
        }
    ...
    if(compute_fp){
        ...
        for(int32_t i=0; i<dc.size(); i++){
            for(int32_t j=0; j<det.size(); j++){
                ...
                if(overlap>MIN_OVERLAP[metric][current_class]){
                    ...
                    if((it = find(stat.fps.begin(),stat.fps.end(),i))!=stat.fps.end()){
                        stat.fps.erase(it);
                    }
        ...
        // get fps_bst by calculating overlap
        for(int32_t i=0;i<det.size();i++){
            // skip det that is not in fps
            if(find(stat.fps.begin(),stat.fps.end(),i)==stat.fps.end())continue;
            for(int32_t j=0;j<gt.size();j++){
                if(ignored_gt[j])
                    continue;
                double overlap = boxoverlap(det[i], gt[j], -1);
                if(overlap>=MIN_OVERPLAP_LEVELS[0]){
                    stat.fps_bst.push_back(i);
                    break;
                }
            }
        }
...
/*=======================================================================
  EVALUATE CLASS-WISE
  =======================================================================*/
vector<string> generateFileNames(int32_t img_idx,int32_t t_idx,string root_dir){
    ...
    sprintf(threshold_str,"/%d",t_idx);

    sprintf(img_name,"/%06d.txt",img_idx);

    basename+=string(threshold_str);
    basename+=string(img_name);
    // fps
    string filename1 = "";
    filename1+=root_dir;
    filename1+="/fps";
    filename1+=threshold_str;
    string threshold1_dir = filename1;
    filename1+=string(img_name);
    // fns
    string filename2 = "";
    filename2+=root_dir;
    filename2+="/fns";
    filename2+=threshold_str;
    string threshold2_dir = filename2;
    filename2+=string(img_name);
    // fps_bst
    string filename3 = "";
    filename3+=root_dir;
    filename3+="/fps_bst";
    filename3+=threshold_str;
    string threshold3_dir = filename3;
    filename3+=string(img_name);
    // fns_bst
    string filename4 = "";
    filename4+=root_dir;
    filename4+="/fns_bst";
    filename4+=threshold_str;
    string threshold4_dir = filename4;
    filename4+=string(img_name);

    filenames.push_back(filename1);
    filenames.push_back(filename2);
    filenames.push_back(threshold1_dir);
    filenames.push_back(threshold2_dir);

    filenames.push_back(filename3);
    filenames.push_back(filename4);
    filenames.push_back(threshold3_dir);
    filenames.push_back(threshold4_dir);

    return filenames;
}

void saveIdxInfo(tPrData tmp,int32_t img_idx, int32_t t_idx, string root_dir){
    if(root_dir==""){
        // don't save!
        return;
    }
    vector<string> filenames = generateFileNames(img_idx,t_idx,root_dir);
    string threshold1_dir = filenames[2];
    string threshold2_dir = filenames[3];
    string threshold3_dir = filenames[6];
    string threshold4_dir = filenames[7];
    if(access(threshold1_dir.c_str(),0)==-1){
        system(("mkdir -p "+threshold1_dir).c_str());
    }

    FILE* fp = fopen(filenames[0].c_str(),"w+");
    /* if(fp==0){ */
    // FILE* fp = fopen(filenames[0].c_str(),"w+");
    // if(fp==0){
    ...
    /* } */
    if(access(threshold2_dir.c_str(),0)==-1){
        system(("mkdir -p "+threshold2_dir).c_str());
    }
    FILE* fn = fopen(filenames[1].c_str(),"w+");
    /* if(fn==0){ */
    // FILE* fn = fopen(filenames[1].c_str(),"w+");
    // if(fn==0){
    ...
    // }
    /* } */
    if(access(threshold3_dir.c_str(),0)==-1){
        system(("mkdir -p "+threshold3_dir).c_str());
    }
    FILE* fp_bst = fopen(filenames[4].c_str(),"w+");

    if(access(threshold4_dir.c_str(),0)==-1){
        system(("mkdir -p "+threshold4_dir).c_str());
    }
    FILE* fn_bst = fopen(filenames[5].c_str(),"w+");


    for(int i=0;i<tmp.fps.size();i++){
        fprintf(fp,"%d\n",tmp.fps[i]);
    }

    for(int i=0;i<tmp.fns.size();i++){
        fprintf(fn,"%d\n",tmp.fns[i]);

    }

    for(int i=0;i<tmp.fps_bst.size();i++){
        fprintf(fp_bst,"%d\n",tmp.fps_bst[i]);
    }

    for(int i=0;i<tmp.fns_bst.size();i++){
        fprintf(fn_bst,"%d\n",tmp.fns_bst[i]);
    }
...
void decorateDir(string& analysis_dir,DIFFICULTY difficulty){
    if(analysis_dir==""){
        //ignore it
        return;
    }
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
        ...
        DIFFICULTY difficulty, METRIC metric,string analysis_dir="",vector<int32_t>*indices=0) {

    decorateDir(analysis_dir,difficulty);
    ...
    for (int32_t i=0; i<groundtruth.size(); i++){
        ...
        for(int32_t t=0; t<thresholds.size(); t++){
            ...
            saveIdxInfo(tmp,(*indices)[i],t,analysis_dir);
...
bool eval(string gt_dir, string result_dir, Mail* mail){
    ...
    string analysis_dir = result_dir + "/analysis";
    // string fps_dir = analysis_dir+ "/fps";
    // string fns_dir = analysis_dir + "/fns";

    // create output directories
    system(("mkdir " + plot_dir).c_str());

    // some analysis dir
    // system(("mkdir " + analysis_dir).c_str());
    // system(("mkdir " + fps_dir).c_str());
    // system(("mkdir " + fns_dir).c_str());
    ...
    // ignore image and ground
    eval_image.assign(eval_image.size(), false);
    eval_ground.assign(eval_ground.size(), false);
    ...
    for (int c = 0; c < NUM_CLASS; c++) {
        ...
        if (eval_3d[c]) {
            ...
            vector<double> precision[3], aos[3];
            if(   !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[0], aos[0], EASY, BOX3D,analysis_dir,&indices)
                    || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[1], aos[1], MODERATE, BOX3D,analysis_dir,&indices)
                    || !eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, box3DOverlap, precision[2], aos[2], HARD, BOX3D,analysis_dir,&indices)) {
                mail->msg("%s evaluation failed.", CLASS_NAMES[c].c_str());
                return false;
            }
            fclose(fp_det);
